<?php
    ...
    add_filter( 'the_content', 'pms_filter_content', 10 );
    ...
    function pms_add_restricted_post_preview( $message, $content, $post, $user_ID ) {
        ...
        if( $preview_option == 'trim-content' ) {
            ...
            if( $length !== 0 ) {
                ...
                /**
                 * Get all shortcodes and replace them with a string so that wp_trim_words() doesn't cut them halfway, thus
                 * preserving them
                 *
                 */
                ...
                if ( preg_match_all( '/'. $pattern .'/s', $post->post_content, $matches ) && array_key_exists( 0, $matches ) ) {
                    ...
                    if( !empty( $shortcodes ) && is_array( $shortcodes ) ) {
                        foreach( $shortcodes as $key => $shortcode )
                            $post_content = str_replace( $shortcode, 'pms_shortcode_placeholder_' . $key, $post_content );
                    }
                }
                ...
                // Add the shortcodes back into the preview
                if( !empty( $shortcodes ) && is_array( $shortcodes ) ) {
                    foreach( $shortcodes as $key => $shortcode )
                        $preview = str_replace( 'pms_shortcode_placeholder_' . $key, $shortcode, $preview );
                }
    ...
    add_filter( 'the_content', 'pms_member_upgrade_subscription', 10 );
    ...
    add_filter( 'the_content', 'pms_member_renew_subscription', 10 );
    ...
    add_filter( 'the_content', 'pms_member_cancel_subscription', 10 );
    ...
    add_filter( 'the_content', 'pms_member_retry_payment_subscription', 10 );